    ...
    half clearCoatMask, bool specularHighlightsOff)
{
    ...
    {
        // float roughness = 0.1;
        // BRDFData new_data = (BRDFData)0;
        // new_data.roughness = max(PerceptualRoughnessToRoughness(roughness), HALF_MIN_SQRT);
        // new_data.roughness2 = max(new_data.roughness * new_data.roughness, HALF_MIN);
        // //brdfData.roughness2 = rougness*rougness;
        // new_data.roughness2MinusOne = new_data.roughness2 - half(1.0);
        // new_data.normalizationTerm = new_data.roughness * half(4.0) + half(2.0);
...
half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData)
{
    ...
    //     inputData.positionWS, brdfData.perceptualRoughness, 1.0h);
    // // lightingData.giColor = EnvironmentBRDF(brdfData, inputData.bakedGI,
    // //     1, fresnelTerm);
    // // 球协（漫反射）+环境光（高光）
