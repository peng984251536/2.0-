    ...
    Subshader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                o.faceDorWS = TransformObjectToWorldDir(faceDirOS);
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                float3 NPR_Color = NPR_face(i, lightMain,_ShadowOffset, _DebugParams);
				...
				for (int j = 0; j < transPixelLightCount; ++j)
				{
				    ...
				    float3 NPR_Add_Color = NPR_Add_face(i, light, _ShadowOffset,_DebugParams);
				    All_Add_Color+=NPR_Add_Color;
				}
            	//return float4(All_Add_Color, 1);
                //return float4(NPR_Color, 1);
                return float4(NPR_Color.rgb+All_Add_Color, 1);
                return float4(All_Add_Color, 1);
                
                return float4(NPR_Color.rgb, 1);
            }
            ENDHLSL
        }
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                float outLine = _LightingMap.SampleLevel(sampler_LightingMap,v.uv,0).r;
			    float3 normalOS = v.normal.xyz;
			    #if defined(_SoomthNormal)
			    normalOS = v.tangent.xyz;
			    #endif

			    float edgeWidth = _EdgeWidth * 0.01*saturate(1-outLine);
			    //根据与摄像机的位置坐衰减
