Shader "Unlit/GeoGrassShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //获取 交给 光栅化的顶点
            g2f GetVertex(float4 pos, float3 normal, float4 uv, float4 color,
                          float3 windDir, float windStrength)
            {
                g2f output;
            ...
            void geom(triangle vertOut IN[3]:SV_POSITION, inout TriangleStream<g2f> triangle_stream)
            {
                ...
                float4 normalWS = mul(mul(sphereMatrix,  bendRoationMatrix), float4(0, 1, 0, 0));
            ...
            float4 frag(g2f i) : SV_Target
            {
                ...
                //stylizedColor = pow(stylizedColor,2.2); 
                ...
                float3 diffuse = texColor * NdotL * _MainLightColor*_GrassColor.rgb ;
                diffuse = lerp(diffuse,stylizedColor,0.66);
                ...
                //自阴影
                float c = lerp(0.33, 1, i.uv.w);
                float3 finColor = diffuse * c + ambient + specular*_SpecularColor.rgb;
