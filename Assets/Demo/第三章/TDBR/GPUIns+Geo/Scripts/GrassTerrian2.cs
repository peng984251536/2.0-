...
namespace URPLearn{
    ...
    public class GrassTerrian2 : MonoBehaviour
    {
        ...
        private static HashSet<GrassTerrian2> _actives = new HashSet<GrassTerrian2>();

        public static IReadOnlyCollection<GrassTerrian2> actives{
            get{
                return _actives;
            }
        }
        ...
        public ComputeBuffer grassBuffer{
            get{
                ...
                for(var j = 0; j < indices.Length / 3; j ++){
                    ...
                    //面得到法向
                    var normal = GrassUtil2.GetFaceNormal(v1,v2,v3);

                    //计算up到faceNormal的旋转四元数
                    var upToNormal = Quaternion.FromToRotation(Vector3.up,normal);

                    //三角面积
                    var arena = GrassUtil2.GetAreaOfTriangle(v1,v2,v3);
                    ...
                    for(var i = 0; i < countPerTriangle; i ++){
                        ...
                        var positionInTerrian = GrassUtil2.RandomPointInsideTriangle(v1,v2,v3);
