    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Frag(Varyings i) : SV_Target
            {
                ...
                //return dot(_VolumeLightDir.xyz,normalize(_ConeBParams.xyz*_ConeAParams.xyz))>0.9;
                float3 startPos = curPos + dir * dis.x;//第一个交点
                float3 startPos2 = curPos + dir * dis.y;//第二个交点
                
                float d1 = distance(startPos,_ConeAParams.xyz);
                float atten1 = 1/pow(2.71828,_VolumeLightParams.w*d1);

                float3 dir01 = normalize(startPos-_ConeAParams.xyz) ;
                float3 light_x = normalize(_VolumeLightDir.xyz);
                float3 light_z =normalize(cross(light_x,dir01)) ;
                float3 light_y = normalize(cross(light_z,light_x));
                float3x3 worldToLight = float3x3
                (
                    light_x.x,light_x.y,light_x.z,
                    light_y.x,light_y.y,light_y.z,
                    light_z.x,light_z.y,light_z.z
                );
                float3x3 lightToWorld = float3x3
                (
                    light_x.x,light_y.x,light_z.x,
                    light_x.y,light_y.y,light_z.y,
                    light_x.z,light_y.z,light_z.z
                );
                //return float4(i.posWS,1);
                // float3 startPosLS = mul(worldToLight,startPos);
                // float3 startPosLS02 = float3(startPosLS.x,-startPosLS.y,startPosLS.z);
                // //float3 startPosWS02 = mul(lightToWorld,startPosLS02);
                // float d2 = abs(startPosLS.y)*2; 
                // float atten2 = ;
                // //atten2 = abs(startPosLS.y)/_VolumeLightParams.w;
                // atten2 = dot(startPosLS,startPosLS02);
                float d2 = distance(startPos,startPos2);
                float d_max = pow(_ConeBParams.w*_ConeBParams.w+_VolumeLightParams.z*_VolumeLightParams.z,0.5);
                d2 = max(0,d_max-d2);
                float atten2 = 1/pow(2.71828,_VolumeLightParams.w*d2);
                
                return float4(lerp(baseColor.rgb, _VolumeLightColor.rgb, atten2), 1);
            }
            ENDHLSL
        }
    }
