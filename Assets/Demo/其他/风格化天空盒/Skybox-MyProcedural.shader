...
SubShader {
    ...
    Pass {
        ...
        v2f vert (appdata_t v)
        {
            ...
            float3 kSkyTintInGammaSpace = LinearToGamma22(_SkyTint); // convert tint from Linear back to Gamma
            ...
            if(eyeRay.y >= 0)
            {
                ...
                float startAngle = dot(eyeRay, float3(0,1,0));
...
//              for(int i=0; i<int(kSamples); i++)
            ...
            else
            {
                ...
                float lightAngle = dot(_MainLightPosition.xyz, pos);
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
            ...
            OUT.skyColor    = _Exposure * (cIn * getRayleighPhase(_MainLightPosition.xyz, -eyeRay));
            ...
            half lightColorIntensity = clamp(length(_MainLightColor.xyz), 0.25, 1);
                ...
                OUT.sunColor    = kSimpleSundiskIntensityFactor * saturate(cOut * kSunScale) * _MainLightColor.xyz / lightColorIntensity;
                // OUT.sunColor    = kHDSundiskIntensityFactor * saturate(cOut) * _MainLightColor.xyz / lightColorIntensity;
