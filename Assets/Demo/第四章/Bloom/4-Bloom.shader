Shader "4/4_Bloom"
{
    ...
    {
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
        float texelSize = _MySourceTex_TexelSize.y;
        float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);

        // Optimized bilinear 5-tap gaussian on the same-sized source (9-tap equivalent)
        half3 c0 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv - float2(0.0,texelSize * 4.0)));
        half3 c1 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv - float2(0.0,texelSize * 3.0)));
        half3 c2 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv - float2(0.0,texelSize * 2.0)));
        half3 c3 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv - float2(0.0,texelSize * 1.0)));
        half3 c4 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv));
        half3 c5 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv + float2(0.0,texelSize * 1.0)));
        half3 c6 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv + float2(0.0,texelSize * 2.0)));
        half3 c7 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv + float2(0.0,texelSize * 3.0)));
        half3 c8 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MySourceTex, sampler_LinearClamp, uv + float2(0.0,texelSize * 4.0)));
