...
public class MaskBloomPass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        using (new ProfilingScope(cmd, m_ProfilingSampler))
        {
            ...
            //拷贝RT
            cmd.SetGlobalTexture("_SourceTex",renderingData.cameraData.renderer.cameraColorTarget);
            cmd.SetGlobalTexture("_DestTex",bloomRTHandle.Identifier());
            cmd.Blit(cameraRenderTarget, bloomRTHandle.Identifier(),material,4);
            ...
            for (int i = 0; i < mipCount; i++)
            {
                ...
                // descriptor.width = Mathf.Max(3, descriptor.width);
                ...
                cmd.Blit(handles[i * 2].Identifier(), handles[i * 2 + 1].Identifier(), material, 0);

                cmd.SetGlobalTexture(MaskBloomFeature.sourceTex, handles[i * 2 + 1].Identifier());
                cmd.Blit(handles[i * 2 + 1].Identifier(), handles[i * 2 + 2].Identifier(), material, 1);
            }
